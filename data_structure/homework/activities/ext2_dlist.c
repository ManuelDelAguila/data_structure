...
int main (int argc, char **argv) {
    ...
    // Verificar que haya al menos un argumento (además del nombre del programa)
    if (argc < 2) {
    ...
    // Insertar los números recibidos como argumentos
    for (i = 1; i < argc; i++) {
        if ((data = (int *)malloc(sizeof(int))) == NULL)
            return 1;

        *data = atoi(argv[i]); // convertir argumento a entero

        // Insertamos al inicio (antes de la cabeza)
        if (dlist_ins_prev(&list, dlist_head(&list), data) != 0)
            return 1;
    }

    // Mostrar la lista inicial cargada desde argv
    print_list(&list);

    /* --- A partir de aquí el resto de tu código se queda igual --- */

    node = dlist_head(&list);

    for (i = 0; i < 7 && node != NULL; ++i)
        node = dlist_next(node);
